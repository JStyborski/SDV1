                    ...
                    for n in trange(opt.n_iter, desc='Sampling'):
                        for prompts in tqdm(data, desc='data'):
                            ...
                            if opt.use_diffusion and opt.ddim_eta_fwd == 1. and not(opt.save_streamlines):
                                ...
                                samples = sampler.decode(z_enc, c, t_enc, unconditional_guidance_scale=opt.scale_rev, unconditional_conditioning=uc)
                                if opt.save_latents:
                                ...
                                noise_latent, i2nList = JS_img2noise2img.image_to_noise(init_latent, model, sampler, t_enc, cond=c, uncond=uc, cfg_scale=opt.scale_fwd, record_latents=opt.save_streamlines)
                                sampler.ddim_sigmas = to_torch(opt.ddim_eta_rev * torch.sqrt((1 - sampler.ddim_alphas_prev) / (1 - sampler.ddim_alphas) * (1 - sampler.ddim_alphas / sampler.ddim_alphas_prev)))
                                samples, n2iList = JS_img2noise2img.noise_to_image(noise_latent, model, sampler, t_enc, cond=c, uncond=uc, cfg_scale=opt.scale_rev, record_latents=opt.save_streamlines)
                                if opt.save_streamlines:
